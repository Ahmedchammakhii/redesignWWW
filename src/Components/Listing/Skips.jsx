...
const Skips = ({data,error,loading}) => {
  ...
  if (error) return (<h1>Oups {error}</h1>)
    return (
     ...
     {data.map((el,i)=>   <div onClick={e=>{e.currentTarget.style.border="10px solid #fff"; e.currentTarget.firstElementChild.innerHTML="Selected"}}
        id={"image"+i} style={{display:"flex",position:"absolute",alignItems:"center",justifyContent:"center",  width: "70vw", height: "70vh", borderRadius: "2rem", backgroundImage: 'url("/16-yarder-skip.jpg")', backgroundSize: "cover",backgroundPosition:"center",overflow:"hidden" }}>
